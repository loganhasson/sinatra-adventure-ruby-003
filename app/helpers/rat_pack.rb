module Sinatra
  class RatPack
    ...
    def self.enable_auto_views=(set=false, called_from=nil)
      if set
    ...
    def self.enable_implicit_views=(set=false)
      if set
            ...
            after '/*?' do |path|
              ...
              if request.path[1..-1] == path || (response.redirect? && response.location.gsub(RatPack::Configure.root,"") == path)
                ...
                if response.redirect?
                  sanitized_path = response.location.gsub(RatPack.root,"")
                  split_path = sanitized_path.split("/").reject{|e| e.empty?}
                else
                  split_path = request.path.split("/").reject{|e| e.empty?}
                end
                puts "Right Method: #{request.path[1..-1] == path || (reponse.redirect? && response.location.gsub(RatPack::Configure.root,"") == path)}"
